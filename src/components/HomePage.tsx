  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      if (e.touches.length > 0) {
        ...
        if (heroRef.current?.contains(target) || pricingRef.current?.contains(target)) {
          if (heroRef.current) handleMove(touch.clientX, touch.clientY, heroRef.current);
          if (pricingRef.current) handleMove(touch.clientX, touch.clientY, pricingRef.current);
    ...
    const handleClickEvent = (e: MouseEvent | TouchEvent) => {
      const target = e.target as HTMLElement;
      if (heroRef.current?.contains(target)) handleClick(e, heroRef.current);
      if (pricingRef.current?.contains(target)) handleClick(e, pricingRef.current);
    };
    ...
    <div className="w-full bg-gradient-to-br from-[#1A237E] via-[#1A237E]/80 to-[#FF6F61]/15">
      <style>{`
        ...
        .button-container {
          ...
          align-items: center;
          justify-content: center;
          gap: 1rem; /* Matches existing gap-3 for mobile */
          width: 100%;
        }
        @media (min-width: 640px) {
          .button-container {
            flex-direction: row;
            gap: 1.25rem; /* Matches existing gap-5 for sm */
          }
        }
        @media (min-width: 768px) {
          .button-container {
            gap: 1.75rem; /* Matches existing gap-7 for md */
          }
        }
      ...
      <section ref={heroRef} className="w-full py-8 sm:py-12 md:py-16 lg:py-20 xl:py-28 relative overflow-hidden perspective-1000">
        ...
        <div className="container px-4 sm:px-6 md:px-8 lg:px-12 max-w-full sm:max-w-7xl mx-auto flex flex-col items-center relative z-20">
          ...
          <div className="text-center mb-8 sm:mb-10 md:mb-12 w-full">
            ...
            <div className="button-container mb-8 sm:mb-10 md:mb-12">
              <Button
